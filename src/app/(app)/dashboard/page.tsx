    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          ...
          <p className="text-2xl font-bold text-zinc-100">{stats.win_rate.toFixed(0)}%</p>
          <p className="text-xs text-zinc-600 mt-1">{stats.total_trades} trades</p>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500 mb-1">Total trades</p>
          <p className="text-2xl font-bold text-zinc-100">{stats.total_trades}</p>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <p className="text-xl font-bold text-emerald-400">{formatCurrency(stats.max_profit)}</p>
          </div>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <p className="text-xl font-bold text-red-400">{formatCurrency(Math.abs(stats.max_loss))}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div className="lg:col-span-3 rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
          <p className="text-sm font-medium text-zinc-300 mb-4">Equity curve (YTD)</p>
          <PnlChart data={chartData} />
        </div>
        <div className="lg:col-span-2 rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
          <p className="text-sm font-medium text-zinc-300 mb-4">Recent trades</p>
